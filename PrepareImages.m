    ...
    if IsLegitDir(categories(classes))
        ...
        for scene = 1:length(category_scenes)
            if IsLegitDir(category_scenes(scene))
                ...
                train_images_idx = randperm(length(images_in_dir)-2, 100) + 2;
...
save('image_info_15_train100.mat', 'test_images', 'test_scene_labels', 'test_category_labels', 'train_images', 'train_scene_labels', 'train_category_labels','-v7.3');
end
